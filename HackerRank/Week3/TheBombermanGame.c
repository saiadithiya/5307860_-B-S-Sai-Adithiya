 ...
 *
 */
char** createGrid(int r, int c, char** prev) {
...
char** bomberMan(int n, int r, char** grid, int* result_count) {
    ...
    // First detonation
    char** after_first = createGrid(r, c, grid);

    // Second detonation
    char** after_second = createGrid(r, c, after_first);

    // Cycle logic: repeats every 4
    char** result;
    if (n % 4 == 3) {
        result = after_first;
    } else {
